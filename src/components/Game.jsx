...
const Game = () => {
    ...
    const crash = useCallback(async () => {
        ...
        if(gameLogicRef.current) {
            ...
            for (let i = 0; i < 25; i++) {
                gameLogicRef.current.particles.push({ x: gameLogicRef.current.bird.x, y: gameLogicRef.current.bird.y, vx: (Math.random() - 0.5) * 15, vy: (Math.random() - 0.5) * 15, life: 1, color: `hsl(${Math.random() * 60 + 15}, 80%, 60%)`});
            }
        }
